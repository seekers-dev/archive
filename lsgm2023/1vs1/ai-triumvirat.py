"""
AI by Jonas, Bela and Karl in the Seekers 1vs1 LSGM Tournament 2023; 1st place
"""
...
def decide(seekers: list[Seeker], other_seekers: list[Seeker], all_seekers: list[Seeker], goals: list[Goal],
    ...
    # for goal in goals:
    #     goal.time_owned = goal.time_owned
    ...
    gatherers = [0, 1, 2]  # list(range(len(seekers) - 2))
    pushers = []
    collector = []
    attackram = []
    rams = [4]  # [len(seekers) - 1]
    defend = [3]
        ...
        if s.disabled():
            ...
            if world.torus_distance(s.position, goal.position) < 42:
                ...
                if vectordiff(world.torus_direction(s.position, goal.position) , world.torus_direction(s.position,
                                                                                              own_camp.position)) \
                        < 0.5:
                    # magforce > world.nearest_seeker(pos=g.position, seekers = other_seekers).magnetic_force(world,g.position).norm():
    ...
    for i in defend:
        s = seekers[i]
        targets[i] = enemy_camp.position  # collision_avoidance(s,enemy_camp.position,world,seekers )
    ...
    #     nearest_goal = world.nearest_goal(s.position, goals)
    #     if world.torus_distance(s.position, nearest_goal.position) < 60 and \
    #             vectordiff(world.torus_direction(s.position, nearest_goal.position), world.torus_direction(s.position,
    #                                                                                             own_camp.position)) \
    #             < 0.5 and \
    #             nearest_goal.velocity.norm() * 49 < world.torus_distance(nearest_goal.position, own_camp.position) and \
    #             world.torus_distance(nearest_goal.position, own_camp.position) < 200 and not s.disabled() and not i in gatherers:
    #         s.set_magnet_repulsive()
    #         print("push")
    ...
    #     for i,s in enumerate(own_seekers_copy):
    #         for j, t in enumerate(own_seekers_copy):
    #             if world.torus_distance(s.position,t.position) < s.radius + t.radius and i != j and not i in collided:
    #                 targets[i] = (targets[i] - seekers[i].position).rotated() + seekers[i].position
    #                 collided.append(i)
    #         if not i in aggressive:
    #             for j, t in enumerate(other_seekers_copy):
    #                 if world.torus_distance(s.position,
    #                                         t.position) < s.radius + t.radius and i != j and not i in collided:
    #                     targets[i] = (targets[i] - seekers[i].position).rotated() + seekers[i].position
    #                     collided.append(i)
    #
    # if len(collided) > 0:
