...
def decide(own_seekers: list[Seeker], other_seekers: list[Seeker], all_seekers: list[Seeker], goals: list[Goal],
    ...
    for i in range(2,4):
        # Movement
        own_seekers[i].target = Vector(other_camp.position.x + math.sin((i - 2) * 1 / 1 * math.pi + 0 * passed_time / (TARGET_RADIUS * 4 / 9)) * TARGET_RADIUS,
                                        other_camp.position.y + math.cos((i - 2) * 1 / 1 * math.pi + 0 * passed_time / (TARGET_RADIUS * 4 / 9)) * TARGET_RADIUS)        
        # Magnet control
