...
### NERVER: fährt im Radius 100 um das genertor und rammt den nächsten


    if world.torus_distance(own_seekers[0].position,other_camp.position) < 130:
        dist_list = []
        for idx, enemy in enumerate(other_seekers):
            dist = world.torus_distance(own_seekers[0].position, other_seekers[idx].position)
            dist_list.append((dist,idx))
            dist_list.sort()

            closest_idx = dist_list[0][1]
            # print("closest idx: " ,closest_idx)
            # print("dist: " , dist)

            if other_seekers[closest_idx].magnet != 0:

                goals[0] = other_seekers[closest_idx]
                ...
                own_seekers[0].orbit_angle += 0.02
    ...
    for i, s in enumerate(own_seekers):  # i is the index of the seeker and s is the seeker object
        ...
        if dist < 60:  # decides if seeker is close enough to the goal
            ...
            abs = world.torus_distance(s.position,own_camp.position)
            # print("Abstand zum Tor" , abs)

            if abs < 30:
                away_vector = s.position -own_camp.position

                away_vector = away_vector.normalized()



                s.target = s.position + away_vector * 100
            else:
